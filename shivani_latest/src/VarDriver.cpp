...
bool VarDriver::readFrameCenters()
{
  ...
  std::cout << "DEBUG: dataPath = " << dataPath << std::endl;
  std::vector<std::string> filenames = FileList(dataPath);
  std::string centerFilename;
  for (std::size_t i = 0; i < filenames.size(); ++i) {
		//std::cout << "DEBUG - Filenames[i] => " << filenames[i] << std::endl;
    ...
    std::string suffix = Extension(filename);
		...
		//std::cout << "DEBUG: Extension = " << suffix << std::endl;
    if (suffix == "cen") {
      // Match to centerfile
  ...
  // Get the date from the filename
	std::cout << "DEBUG: centerFilename => " <<centerFilename << std::endl;
  std::string datestr = centerFilename.substr(0,8);
  datetime startDate = ParseDate(datestr.c_str(), "%Y%m%d");
	std::cout << "DEBUG: datestr & startDate => " << datestr << ", " << PrintTime(startDate) << std::endl;
  ...
  while (std::getline(centerFile, line)) {
    ...
    //QTime time = QTime::fromString(QString::fromStdString(timestr), "HHmmss");
    ...
    FrameCenter center(datetime_, lat, lon, Um, Vm);
		//std::cout << "DEBUG: Added center of " << datetime_ << std::endl;
...
{
	//std::cout << "DEBUG: read_met_obs_file : " << suffix << "  ( " << filename << " ) " << std::endl;
  switch (suffix) {
/*fixme
  case (frd):
    if (!read_frd(filename, metData)) {
      cout << "Error reading frd file" << endl;
      return false;
    }
    break;
  case (cls):
    if (!read_cls(filename, metData)) {
      cout << "Error reading cls file" << endl;
      return false;
    }
    break;
  case (sec):
    if (!read_sec(filename, metData)) {
      cout << "Error reading sec file" << endl;
      return false;
    }
    break;
  case (ten):
    if (!read_ten(filename, metData)) {
    ...
    */
  case (swp):
    if (!read_dorade(filename, metData)) {
    ...
    /*fixme
  case (sfmr):
    if (!read_sfmr(filename, metData)) {
      cout << "Error reading sfmr file" << endl;
      return false;
    }
    break;
  case (wwind):
    if (!read_wwind(filename, metData)) {
      cout << "Error reading wwind file" << endl;
      return false;
    }
    break;
  case (eol):
    if (!read_eol(filename, metData)) {
      cout << "Error reading eol file" << endl;
      return false;
    }
    break;
  case (qscat):
    if (!read_qscat(filename, metData)) {
      cout << "Error reading wwind file" << endl;
      return false;
    }
    break;
  case (ascat):
    if (!read_ascat(filename, metData)) {
      cout << "Error reading wwind file" << endl;
      return false;
    }
    break;
  case (nopp):
    if (!read_nopp(filename, metData)) {
      cout << "Error reading wwind file" << endl;
      return false;
    }
    break;
  case (cimss):
    if (!read_cimss(filename, metData)) {
      cout << "Error reading cimss file" << endl;
      return false;
    }
    break;
  case (dwl):
    if (!read_dwl(filename, metData)) {
      cout << "Error reading dwl file" << endl;
      return false;
    }
    break;
  case (insitu):
    if (!read_insitu(filename, metData)) {
      cout << "Error reading insitu file" << endl;
      return false;
    }
    break;
  case (mtp):
    if (!read_mtp(filename, metData)) {
      cout << "Error reading mtp file" << endl;
      return false;
    }
  case (mesonet):
    if (!read_mesonet(filename, metData)) {
      cout << "Error reading mesonet file" << endl;
      return false;
    }
    break;
  case (classnc):
    if (!read_classnc(filename, metData)) {
      cout << "Error reading classnc file" << endl;
      return false;
    } 
    break;
  case (qcf):
    if (!read_qcf(filename, metData)) {
      cout << "Error reading classnc file" << endl;
      return false;
    }
    break;
  case(aeri):
    if (!read_aeri(filename, metData)) {
      cout << "Error reading aeri file" << endl;
      return false;
    }
    break;
  case(rad):
    if (!read_rad(filename, metData)) {
  ...
  case (cen):
    return false;*/
  case(cfrad):
    if (!read_cfrad(filename, metData)) {
...
/*fixme
  ...
  while (!in.atEnd()) {
    ...
    if (line.startsWith(" Aircraft:")) {
      ...
      // Start reading data
      start = true;
      QDate date = QDate::fromString(QString::fromStdString(datestr), "yyyyMMdd");
      QTime time = QTime::fromString(QString::fromStdString(timestr), "HHmmss");
      datetime = QDateTime(date, time, Qt::UTC);
    } else if (start) {
      MetObs ob;
      ob.setStationName(aircraft);
      std::stringList lineparts = line.split(QRegExp("\\s+"));
...
/*fixme
  ...
  while (!in.atEnd()) {
    ...
    if (line.startsWith("Release Site Type")) {
      std::stringList lineparts = line.split(":");
...
/*fixme
  QTextStream in(&metFile);
  std::string datestr, timestr, aircraft;
  std::ifstreamInfo info(metFile);
  std::string fileName = info.fileName();
  aircraft = fileName.indexOf(QRegExp("HINUE"));
  datestr = fileName.left(8);
  QDate startDate = QDate::fromString(QString::fromStdString(datestr), "yyyyMMdd");
  MetObs ob;
  while (!in.atEnd()) {
    std::string line = in.readLine();
    line.remove(0, 1);
...
bool VarDriver::read_dorade(std::string& filename, std::vector<MetObs>* metObVector)
{
  ...
  for (int i=0; i < swpfile.getNumRays(); i+=rayskip) {
    ...
    //std::cout << "*** Got ray time of: " << PrintDate(rayTime) << " ( " << PrintTime(rayTime) << " ) " << std::endl;
  ...
  //std::cout << "TIME_DEBUG(1) Setting time to " << PrintDate(rayTime) << std::endl;
	ob.setTime(rayTime);
  //auto dt = ob.getTime();
  //std::cout << "TIME_DEBUG(2) Ob Time is " << PrintDate(ob.getTime()) << std::endl;
	metObVector->push_back(ob);
...
/*fixme
  ...
  while (!in.atEnd()) {
    ...
    date = QDate::fromString(QString::fromStdString(datestr), "MMMddyyyy");
    datetime = QDateTime(date, time, Qt::UTC);
    ob.setTime(datetime);
    //std::string datestr = datetime.toString(Qt::ISODate);
...
/* fixme
  ...
  while (!in.atEnd()) {
    ...
    QTime time = QTime::fromString(QString::fromStdString(timestr), "HH:mm:ss.zzz");
    datetime = QDateTime(date, time, Qt::UTC);
    ob.setTime(datetime);
    std::string lat = line.mid(12,5);
    std::string lon = line.mid(18,6);
    std::string speed = line.mid(28,5);
    std::string dir = line.mid(34,6);
    std::string rainprob = line.mid(46,5);
...
/*fixme
  QTextStream in(&metFile);
  MetObs ob;
  std::string unixtime = in.readLine();
  QDateTime datetime;
  datetime = QDateTime::fromTime_t(unixtime.toUInt());
  while (!in.atEnd()) {
    std::string line = in.readLine();
    std::stringList lineparts = line.split(std::string(","));
...
/*fixme
  ...
  ob.setStationName("qscat");
  startDate = QDate::fromString(QString::fromStdString(lineparts[1]), "yyyyMMdd");
...
/*fixme
  ...
  while (!in.atEnd()) {
    line = in.readLine();
    std::stringList lineparts = line.split(QRegExp("\\s+"));
    std::string station = lineparts[0] + " " + lineparts[1];
    ob.setStationName(station);
    QDate date = QDate::fromString(QString::fromStdString(lineparts[2]), "yyyyMMdd");
    QTime time = QTime::fromString(QString::fromStdString(lineparts[3]), "HHmm");
    datetime = QDateTime(date, time, Qt::UTC);
    ob.setTime(datetime);
...
/*fixme
  ...
  while (!in.atEnd()) {
    ...
    if (line.contains("rawfile")) {
      // Get the datetime
      date = QDate::fromString(QString::fromStdString(line.mid(10,8)), "yyyyMMdd");
    } else if (line.contains("%~~")) {
      start = true;
    } else if (start) {
      std::stringList lineparts = line.split(QRegExp("\\s+"));
      QTime time = QTime::fromString(QString::fromStdString(lineparts[0]), "HHmmss");
      datetime = QDateTime(date, time, Qt::UTC);
...
/*fixme
  QTextStream in(&metFile);
  std::string datestr, timestr, platform;
  QDateTime datetime;
  std::ifstreamInfo info(metFile);
  std::string fileName = info.fileName();
  datestr = fileName.left(8);
  ...
  std::vector<const XMLElement* > elementList = XMLGetElements(&config);
	...
	for (const auto element : elementList) {
		...
		for (auto &config : configList) {
			...
			//std::cout << "Inserted : " << tag << ",  " << config->GetText() << std::endl;
		}
	}
...
*/
  ...
  for (std::size_t i = 0; i < configKeys.size(); i++) {
...
bool VarDriver::read_classnc(std::string& filename, std::vector<MetObs>* metObVector)
{
  ...
  //datetime startDate = date::make_zoned("UTC", fromString(fileparts[1], "YYYYMMdd")); 
  //datetime startTime = date::make_zoned("UTC", fromString(fileparts[2], "HHmmss")); 
...
/*fixme
      ...
      std::string obstring = datetime.toString(Qt::ISODate);
...
/*fixme		
  ...
  while (!in.atEnd()) {
    line = in.readLine();
    std::vector<std::string> lineparts; // fixme = line.split(QRegExp(","));
