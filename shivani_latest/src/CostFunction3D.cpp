...
void Stats(const real* Astate, const real* Cstate, const int var, const int nState, const char* message)
{
  double sumA = 0.0;
  double sumC = 0.0;
  ...
  printf("[Stats : %s ] \n", message);
  printf("    sumA      sumB    nState    var\n", message);
  printf("    %g         %g      %d        %d\n", sumA, sumC, nState, var);
}
				...
				real* bgU, real* obs, ReferenceState* ref)
{
  ...
  bgFields = bgU;
	...
	std::cout << "stof conversion..." << std::endl;
  ...
  if (iteration == 1) {
    ...
    if (mishError != NULL) {
      ...
      if(meshError == NULL) {
				...
				cout  << "Before call to SAtransform point #1\n";
#pragma acc data copyin(SBError[0:nState]) copyout(meshError[0:nState])
{
    ...
    if ((*configHash)["load_bg_coefficients"] == "true") {
      ...
      //JMD really expensive comment out for now.
      ...
      cout  << "Before call to SAtransform point #2\n";
#pragma acc data copyin(stateB[0:nState]) copyout(stateA[0:nState])
{
      cout  << "Before call to SAtransform point #2.1\n";
  ...
  for (int var = 0; var < varDim; var++) {
    ...
    if (varScale) {
      ...
      cout << "Variable " << var << " RMS = " << varScale << "\t BG Error = " << bgError[var] << " ( " << errPct << " %)" << endl;
    } else {
...
#pragma acc data create(stateB[0:nState])
{
  ...
  cout << "CostFunction3D::initState SCtransform() \n";
...
#pragma acc data copyin(state[0:nState]) copyout(gradient[0:nState])
{
  ...
  FFtransform(stateC, stateA);       // running on CPU
  ...
  cout << "CostFunction3D::funcGradient \n";
...
real CostFunction3D::funcValueAndGradient(real* state, real *gradient)
{
  ...
  qIP = 0.;
  obIP = 0.;
  cout << "CostFunction3D::funcValueAndGradient #1\n";

//#pragma acc data copyin(state[0:nState]) copyout(gradient[0:nState]) create(stateB[0:nState],stateC[0:nState])
//#pragma acc data copyin(state[0:nState]) copyout(gradient[0:nState]) create(stateC[0:nState])
#pragma acc data copyin(state[0:nState]) copyout(gradient[0:nState])
{
  ...
  cout << "CostFunction3D::funcValueAndGradient #2\n";
  #pragma acc data copyin(stateB[0:nState])
  {
    SCtransform(stateB, stateC);
  }
  //calc gradient
...
void CostFunction3D::calcHTransposeOld(const real* yhat, real* Astate)
{
  ...
  real val;
  GPTLstart("CostFunction3D::calcHTranspose");
#pragma acc data copyin(yhat[0:mObs]) copyout(Astate[0:nState])
{
  // Clear the Astate
  GPTLstart("CostFunction3D::calcHTranspose:zero Astate");
  #pragma acc parallel loop gang vector vector_length(32) private(n)  //[3]
  for (n = 0; n < nState; n++) {
    Astate[n] = 0.0;
  }
  GPTLstop("CostFunction3D::calcHTranspose:zero Astate");

 GPTLstart("CostFunction3D::calcHTranspose:calc tempHval");
  #pragma omp parallel for private(m,val,j) //[4]
  #pragma acc parallel loop gang vector vector_length(32) private(m,val,j) //[4]
  for(m=0; m<mObs; ++m) {
    //val = yhat[m] * obsVector[m*(7+varDim*derivDim)+1];
    val = yhat[m] * obsData[m];
    for(j=IH[m]; j<IH[m+1]; ++j) {
      tempHval[j] = H[j] * val;
    }
  }
 // cout << "End calcHTranspose\n\n";
 GPTLstop("CostFunction3D::calcHTranspose:calc tempHval");
}

 GPTLstart("CostFunction3D::calcHTranspose:update Astate");
 // #pragma acc parallel loop gang vector private(index,i,tmp) //[5] This gives incorrect answers when used.
  for(i=0; i<IH[mObs]; ++i) {Astate[JH[i]]+=tempHval[i];}
 GPTLstop("CostFunction3D::calcHTranspose:update Astate");
...
#pragma acc parallel loop vector_length(32) private(n) //[5.1]
    ...
    for (int var = 0; var < varDim; var++) {
      //GPTLstart("CostFunction3D::SCtransform:FK");
      //FK
//TIME: 11.64 ms
...
//cout << "var: " << var << "\n";
//Stats(Astate, Cstate, var, nState, "Pre-Launcher");
#pragma acc host_data use_device(Astate,Cstate) 
{
object.launcherFunction_SCtransform(Astate,Cstate,var,*this, kFilter->order, kFilter->alpha, kFilter->beta, kFilter->Sn);
}
//Stats(Astate, Cstate, var, nState, "Post-Launcher");
...
#endif
      //GPTLstop("CostFunction3D::SCtransform:FK");
      //cout << "SCtransform: FJ\n";
      //GPTLstart("CostFunction3D::SCtransform:FJ");
      //FJ
//TIME: 2.95 ms
...
#pragma acc parallel loop gang vector vector_length(32) private(iIndex,jIndex,kIndex,index,jTemp,jq,js) //[5.3]
      ...
      //GPTLstop("CostFunction3D::SCtransform:FJ");
      //cout << "SCtransform: FI\n";
      //GPTLstart("CostFunction3D::SCtransform:FI");
      //FI
//TIME: 2.94 ms
...
#pragma acc parallel loop gang vector vector_length(32) private(iIndex,jIndex,kIndex,index,iTemp,iq,is) //[5.4]
  ...
  //cout << "At the end of CostFuncton3D::SCtransform \n";
}
