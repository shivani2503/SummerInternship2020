...
{
//fixme
//	std::list<std::string> pathparts = filename.split("/");
//	std::list<std::string> fileparts = pathparts.last().split(".");
//	std::string radarname = fileparts[2];
// dummy:
  std::string radarname = "foobar";
	return radarname; // fixme
...
float* Dorade::getRayData(int &ray, const std::string& field)
{
	...
	for (int j=0; j<(rptr->num_param_desc); j++) {
		...
		//fixme fieldName.remove(QRegExp("[\\s+]"));
		if (field == fieldName) {
			// Match
			fldIndex = j;
			break;
...
datetime Dorade::getRayTime(int& ray)
{
  ...
  //auto date = date::years{vptr->year} + date::months{1} + date::days{1} + std::chrono::hours{ryptr[ray].hour} + std::chrono::minutes{ryptr[ray].min} + std::chrono::seconds{ryptr[ray].sec} + std::chrono::milliseconds{ryptr[ray].msec};
  //std::cout << "DATE DIFF: " << (date-begin0) << std::endl;
  //auto fm = date - beginE;
  //std::cout << "Other DIFF: " << typeid(beginE).name() << std::endl;
