...
bool VarDriver3D::initialize(const XMLNode& configuration)
{
  ...
  cout << "Initializing SAMURAI 3D (from XML)" << endl;
...
bool VarDriver3D::validateDriver()
{
  ...
  /* Set the data path */
  // Note we can't arrange the filter or sorting here, to my knowledge
  dataPath = configHash["data_directory"];
  if (!DirectoryExists(dataPath)) {  // if it doesn't exist, error out
    cout << "Can't find data directory: " << configHash["data_directory"] << endl;
    return false;
  }
  ...
  if (!DirectoryExists(outputPath)) {
    cout << "Can't find output directory: " << configHash["output_directory"] << endl;
    return false;
  }
  ...
  if (fractlBkgd) { // Grid comes from the fractl file
    ...
    if (loadBG)
      bkgdAdapter = new BkgdStream((dataPath + "/samurai_Background.in").c_str());
...
bool VarDriver3D::gridDependentInit()
{
  ...
  std::string refSounding = std::string(configHash["ref_state"]);
...
{
	// bpd6 - not sure whether we're JUST comparing times, or if dates matter?  Seems not, but I'm going to assume that's a bug and they want to..
  //datetime reftime = std::chrono::time_point<std::chrono::system_clock>{};// ::from_time_t(configHash["ref_time"], "%Y%m%d %T"));
  datetime reftime = ParseDate(configHash["ref_time"].c_str(), "%H:%M:%S");
	//std::cout << "DEBUG: configHash['ref_time'] = " << configHash["ref_time"] << std::endl;
	//std::cout << "DEBUG: reftime = " << PrintTime(reftime) << std::endl;

  for (unsigned int fi = 0; fi < frameVector.size(); fi++) {
    datetime frametime = frameVector[fi].getTime();
		//std::cout << "DEBUG1: Comparing " << PrintTime(reftime) << " & " << PrintTime(frametime) << std::endl;
		//std::cout << "DEBUG2: Comparing " << Time(reftime) << " & " << Time(frametime) << std::endl;
...
bool VarDriver3D::run()
{
	...
	while (iter <= maxIter) {
		...
		cout << "Outer VarDriver3D Loop Iteration: " << iter << endl;
		START_TIMER(timei);
...
bool VarDriver3D::finalize()
{
	...
	delete[] obs;
	delete[] bgU;
	//BUG comment out this delete for now due to a bug with OPENACC
	//BUG delete obCost3D;
	delete refstate;
	return true;
}
...
bool VarDriver3D::preProcessMetObs()
{
  ...
  for (std::size_t i = 0; i < filenames.size(); ++i) {
    ...
    if (suffix == "nc") {	// cfrad file?
      // BPD6 - Error - commenting out the regex search since this isn't correct, but will let us test the Beltrami data
			std::regex re(".*crad.*\\.nc");
		...
		int x = dataSuffix[suffix];
    cout << "Processing " << file << " of type " << suffix << " ( code : " << x << " ) " << endl;
    attemptedFiles++;
...
//      std::cout << "**DEBUG[obt]** " << PrintDate(obTime_ob) << " vs. " << PrintDate(startTime_ob) << " - " << PrintDate(endTime_ob) << std::endl;
//      std::cout << "**DEBUG[int]** " << obTime << " vs. " << startTime << " - " << endTime << std::endl;
	...
	continue;
      ...
      //wrong!auto fi = startTime - obTime;
      //auto fi = obTime - startTime;
      if ((fi < 0) or (fi > (int)frameVector.size())) {
	cout << "**Time problem with observation " << fi << ", " << startTime << ", " << obTime << endl;
	//std::cout << "tcstart2: " << PrintDate(startTime_ob) << ", tcend: " << PrintDate(endTime_ob) << ", obTime: " << PrintDate(obTime_ob) << std::endl;
	timeProblem++;
	continue;
      }
	...
	if (obRadius == 0.0) {
      ...
      //std::cout << "bpd6 - setting obTime to 0." << std::endl;
      varOb.setTime(obTime);
      //varOb.setTime(obTime.time_since_epoch()); ref_time
      ...
      case (MetObs::radar):
	{
	  ...
	  if (configHash["horizontal_radar_appx"] == "true") wWgt = 0;
...
#pragma omp parallel for
        ...
        // NCAR edit - disabling XYZ/RTZ mode check now for performance
//		    if (runMode == XYZ) {
...
//			if (runMode == XYZ) {
  ...
  Observation varOb;
  std::cout << "bpd6 - creating ob and setting time to " << std::stoi(configHash["ref_time"]) << std::endl;
...
bool VarDriver3D::loadPreProcessMetObs()
{
    ...
    std::string obFilename = dataPath + "/samurai_Observations.in";
		std::cout << "DEBUG: Opening " << obFilename << std::endl;
...
{
  if (runMode == XYZ) {
		// Apparently QHash automatically converts non-existent entries to zeros?  That's strange, but we'll test then convert:
		if (configHash.find("output_pressure_increment") != configHash.end()) {
    	if (std::stof(configHash["output_pressure_increment"]) > 0) {
				std::cout << "DEBUG: Making XYP costfunction" << std::endl;
      ...
      obCost3D = cf;
			std::cout << "DEBUG: Making COAMPS costfunction" << std::endl;
    } else {
      obCost3D = new CostFunctionXYZ(projection, obVector.size(), bStateSize);
			std::cout << "DEBUG: Making XYZ costfunction" << std::endl;
    }
  } else if (runMode == RTZ) {
    obCost3D = new CostFunctionRTZ(projection, obVector.size(), bStateSize);
		std::cout << "DEBUG: Making RTZ costfunction" << std::endl;
  }
