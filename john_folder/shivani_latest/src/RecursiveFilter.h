...
public:
	...
	bool CUDAfilterArray(double* array, double *q, double *s, const int& arrLength, const int &order);
...
public:
	...
	double CUDAfactorial(const double& max); 		
        #pragma acc routine seq
	void solveBC(double* A, double* B, double S[5][5]);	
	void CUDAsolveBC(double* A, double* B, double S[5][5]);	
};
